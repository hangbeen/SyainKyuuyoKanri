package menu.payroll.PayrollInputManagement.Model;

public class TotalShiharai {
    private int totalKingaku; // 지급 항목 ID (支給項目 ID)
    private int shainId; // 사원 ID (社員 ID)
    private double shikyuuzoukyuu; // 지급 총액 (支給総額)
    private double kihonkyu; // 기본급 (基本給)
    private double shokujitane; // 식사 수당 (食事手当)
    private double hoikutane; // 보육 수당 (保育手当)
    private double yakushokutane; // 직책 수당 (役職手当)
    private double sharyouijihi; // 차량 유지비 (車両維持費)
    private double kinzokutane; // 근속 수당 (勤続手当)
    private double touchakutane; // 당직 수당 (当直手当)
    private double shouyo; // 보너스 (賞与)
    private double kyuujitsutane; // 휴일 수당 (休日手当)
    private String kazeiUmu; // 과세 여부 (課税有無)
    private String zessaTani; // 절사 단위 (切り捨て単位)
    private String kintaiIkatsu; // 근태 연결/일괄 지급 (勤怠連携/一括支給)
    private double ikkatsuShikyuugaku; // 일괄 지급액 (一括支給額)
